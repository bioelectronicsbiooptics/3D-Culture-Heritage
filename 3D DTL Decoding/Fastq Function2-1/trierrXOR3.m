function trid = trierrXOR3(mat,XA,XB,XC,tc,tp,Ncount,Nxcount,ncp,nbp,ncc,nbc)
s = size(mat,2);% 밖에서 불러와도
tri_part = mat(1:floor(numel(mat)*2/3/s),:);
xor_part = mat(floor(numel(mat)*2/3/s)+1:end-1,:);
...
%%추가 
%% resize (아마 내용 안에도 tc tp Matrix를 10진수 3단위로)
